...
public class FileEntityDao implements CrudDao {
    ...
    private static final String SELECT_ALL = """
        SELECT uuid, orig_name, type, size_bytes, created_at
        FROM files
        """;
    ...
    public List<FileEntity> findAll() {
        try (Connection conn = ConnectionManager.open();
            ...
            while (rs.next()) {
                ...
                // допустим, FileEntity хранит поле LocalDate date;
                // извлечём его из timestamp created_at
                f.setAddedAt(rs.getTimestamp("addedAt").toLocalDateTime().toLocalDate());
                f.setUpdatedAt(rs.getTimestamp("updated_at").toLocalDateTime().toLocalDate());
                list.add(f);
            }
            return list;
        } catch (SQLException e) {
